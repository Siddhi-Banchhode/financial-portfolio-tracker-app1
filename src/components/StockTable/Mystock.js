...
class Mystock extends Component {
  ...
  componentDidMount() {
    axios
      .get(`https://financial-portfolio-30240.firebaseio.com/mystock.json`)
      .then((db) => {
        const res = db.data;
        if (res === null) {
          this.setState({
          ...
          for (let i = 0; i < Keys.length; i++) {
            ...
            this.StockInfo = (x) => {
              axios
                .get(
                  `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${c_symb}&apikey=TJL7RZ1WR92B9CYO`
                )
                .then((res) => {
                  let Data = [];
                  let Current_p = [];
  ...
  stopTrack = (index) => {
    ...
    let ref = firebase.database().ref("mystock");
    ref
